...
for i, classe in enumerate(classi):
		...
		if classe == 'V':
			...
			pre_window = 25
			post_window = 25
			model = NeuralForecastRNN(learning_rate=0.0005, max_steps=500, batch_size=128)
		elif classe == 'F':
			n = 10
			pre_window = 25
			post_window = 25
			model = NeuralForecastRNN(learning_rate=0.001, max_steps=500, batch_size=128)
		elif classe =='S':
			n = 16
			pre_window = 15
			post_window = 15
		...
		for numero in range(sampling_model):
				...
				if numero == 0:
						...
						title = "rnn generated vs real beat of class " + classe + ".jpg"
				...
				if numero % 1000 == 0:
					...
					savemat("beats_rnn_1.mat", dict_aug_model)


# METRICS

den = len(classi) * sampling_model
output = "output_rnn.txt"

with open(output, "w") as f:

    print(f"AVERAGE TIME FOR CREATING A BEAT WITH RNN: {round(time_passed_model / den, 3)} seconds.", file=f)
    print('', file=f)
    print('AVERAGE METRICS SUMMARY FOR RNN', file=f)
    print('', file=f)
    print('DTW:\t\t ', round(model_dtw / den, 5), file=f)
    print('', file=f)
    print('WASSERSTEIN DISTANCE:\t\t ', round(model_was / den, 5), file=f)
...
df_aug_model = pd.DataFrame(aug_model, columns=columns)
dict_aug_model = {"dataframe": df_aug_model.to_dict("list")}
savemat("beats_rnn.mat", dict_aug_model)
